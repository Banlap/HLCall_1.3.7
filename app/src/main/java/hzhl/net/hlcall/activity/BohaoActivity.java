...
public class BohaoActivity extends BaseActivity implements SensorEventListener {
    ...
    protected void init() {
        ...
        mListener = new CoreListenerStub() {
           ...
           /* Logger.d("state:" + state +"  getCurrentParams: "
                ...
                //banlap: bug fixes: 修复拨号后双方无声音情况 --2020.10.15
    ...
    protected void onResume() {
        ...
        if (core != null) {
            ...
            float dx = 1;
            float dy = 1;
            if (metrics.heightPixels/(float)metrics.widthPixels > videoHeight/(float)videoWith){
    ...
    @OnClick({R.id.iv_close_bohao, R.id.tv_mute, R.id.tv_keyboard, R.id.tv_mianti, R.id.tv_record
            , R.id.ll_call_bohao, R.id.rl_bohao_top, R.id.tv_to, R.id.tv_transfer, R.id.iv_del
            ...
            R.id.iv_switch_camera, R.id.tv_video, R.id.tv_more,R.id.tv_stop})
    ...
    public void setButtonEnable(Boolean f){
        ...
        findViewById(R.id.tv_stop).setEnabled(f);
    ...
    private void switchMenuPanel(){
        ...
        if(mIsVideoCallModel){
            ...
            iv_close_bohao.setVisibility(visibility == View.VISIBLE ? View.INVISIBLE : View.VISIBLE);
    ...
    private void setVideoEnabled(Boolean mIsVideo){
        runOnUiThread(() -> {
                if (mIsVideo != null && mIsVideo) {
                    ...
                    //mIsVideoCallModel = true;
                    tvVideo.setSelected(true);
                    rl_bohao_top.setGravity(Gravity.LEFT|Gravity.BOTTOM);
                    //LinphoneUtils.reloadVideoDevices();
                    //CallManager.getInstance().acceptCallVideo(true);
                } else {
                    //CallManager.getInstance().acceptCallVideo(false);
                    ...
                    //mIsVideoCallModel = false;
                    tvVideo.setSelected(false);
                    rl_bohao_top.setGravity(Gravity.CENTER_HORIZONTAL|Gravity.BOTTOM);
                }
